...
from utils.fleet_hosts import (  # noqa: E402
    ...
    HOSTS_BLOCK_END as END,
)

HOSTS_PATH = Path("/etc/hosts")
RESOLVED_DROPIN_DIR = Path("/etc/systemd/resolved.conf.d")
RESOLV_CONF = Path("/etc/resolv.conf")
# BEGIN/END/DEFAULT_DOMAIN are imported above, not re-typed: this script is the
# block's WRITER and mini's membership leg is a READER, so the delimiters have
# two consumers and belong in one place (honest_failure_modes #5).
