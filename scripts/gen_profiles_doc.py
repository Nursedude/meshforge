...
w("2. **Hiding is a VIEW, never a capability.** Every gated menu carries a")
w("   first row reading `Show all  N hidden by profile 'x'`. One keystroke")
w("   restores the full surface for the session. It is the FIRST row")
w("   because a section list can be taller than the terminal — measured on")
w("   a 24x80 PTY, a bottom-placed row scrolled out of sight.")
