        ...
        with self._lock:
            ...
            if event == "connected":
                ...
                if self._connected[service]:
                    # Accumulate uptime. Clamp the delta ≥ 0 (Pri-13-class,
                    # gateway review 2026-07-23): these are wall-clock deltas
                    # and a backward NTP step on the fleet's RTC-less Pis would
                    # otherwise subtract from accumulated uptime (honest_failure_
                    # modes #6). Display/summary only, so a clamp — not a
                    # monotonic rework that would break the wall-clock
                    # last_connected/last_disconnected timestamps — suffices.
...
class DeliveryTracker:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # "0% confirmed = total failure" (Issue #74; aligns with the
        # DeliveryCounters sentinel: rate = None iff sent == 0).
