"""
Tactical Operations Handler — SITREP, TASK, CHECKIN, zones, QR, map, ATAK export.

Converted from tactical_ops_mixin.py as part of the mixin-to-registry migration.
Dispatched from the main menu via tag 't', gated by 'tactical' feature flag.
"""
...
class TacticalOpsHandler(BaseHandler):
    """TUI handler for tactical operations."""
    ...
    def __init__(self):
        super().__init__()
        self._tactical_timeline = None
        self._tactical_settings = None
    ...
    def _get_tactical_settings(self) -> dict:
        ...
        if self._tactical_settings is None:
    ...
    def _tactical_send_sitrep(self):
        ...
        situation = self.ctx.dialog.inputbox(
            "SITREP — Situation",
        ...
        actions = self.ctx.dialog.inputbox(
            "SITREP — Actions Taken",
            "Actions taken (or leave empty):"
        ) or ""

        resources = self.ctx.dialog.inputbox(
    ...
    def _tactical_send_task(self):
        ...
        description = self.ctx.dialog.inputbox(
            "TASK — Description",
            "Task description:"
        )
        if not description:
        ...
        assignee = self.ctx.dialog.inputbox(
            "TASK — Assignee",
    ...
    def _tactical_checkin(self):
        ...
        status = self.ctx.dialog.menu("CHECK-IN — Status", "Your status:", status_choices)
        if not status:
            status = "ok"

        count_str = self.ctx.dialog.inputbox(
            "CHECK-IN — Personnel",
            "Number of personnel:",
            init="1",
        )
    ...
    def _tactical_mark_zone(self):
        ...
        name = self.ctx.dialog.inputbox("ZONE — Name", "Zone name:")
        if not name:
            return
        ...
        zone_type = self.ctx.dialog.menu("ZONE — Type", "Zone type:", type_choices)
        if not zone_type:
            return

        lat_str = self.ctx.dialog.inputbox("ZONE — Center Latitude", "Latitude (decimal):")
        lon_str = self.ctx.dialog.inputbox("ZONE — Center Longitude", "Longitude (decimal):")
        ...
        radius_str = self.ctx.dialog.inputbox(
            "ZONE — Radius",
    ...
    def _tactical_send_resource(self):
        ...
        name = self.ctx.dialog.inputbox("RESOURCE — Name", "Resource name:")
        if not name:
            return
        ...
        resource_type = self.ctx.dialog.menu(
            "RESOURCE — Type", "Resource type:", type_choices
        )
        if not resource_type:
            return

        qty_str = self.ctx.dialog.inputbox("RESOURCE — Quantity", "Quantity:", init="1")
        try:
    ...
    def _tactical_map_view(self):
        ...
        try:
            ...
            self.ctx.dialog.msgbox("Map Error", f"Failed to generate map: {e}")
    ...
    def _tactical_export(self):
        ...
        choice = self.ctx.dialog.menu("Export Format", "Choose export format:", choices)
        if choice is None or choice == "back":
            return
        ...
        if choice == "kml":
            ...
            try:
                ...
                self.ctx.dialog.msgbox("Export Error", f"CoT export failed: {e}")
    ...
    def _tactical_timeline_view(self):
        ...
        if not events:
            self.ctx.dialog.msgbox("Timeline", "No tactical events recorded yet.")
            return
    ...
    def _tactical_compliance_mode(self):
        ...
        if choice and choice in ('C', 'S'):
            ...
            self.ctx.dialog.msgbox("Mode Set", f"Tactical messages will use: {mode_name}")
