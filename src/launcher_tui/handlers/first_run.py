...
class FirstRunHandler(BaseHandler):
    ...
    # -- Delegated to _first_run_wizard.py ---------------------------------
    # The following methods delegate to standalone functions in the private
    # helper module for file size compliance (CLAUDE.md #6).

    def _wizard_step_usb_manual(self):
        """Fallback: manually select USB device when no templates available."""
        from ._first_run_wizard import wizard_step_usb_manual
        wizard_step_usb_manual(self)

    def _wizard_step_network_config(self):
        """Configure network connection to remote meshtasticd."""
        from ._first_run_wizard import wizard_step_network_config
        wizard_step_network_config(self)

    def _wizard_step_region(self):
        """Step 3: Select regulatory region."""
        from ._first_run_wizard import wizard_step_region
        wizard_step_region(self)

    def _find_usb_serial_devices(self) -> List[Dict[str, str]]:
        """Find USB serial devices."""
        from ._first_run_wizard import find_usb_serial_devices
        return find_usb_serial_devices(self)

    def _apply_hardware_config(self, hw_config: dict):
        """Apply a hardware configuration file."""
        from ._first_run_wizard import apply_hardware_config
        apply_hardware_config(self, hw_config)

    def _create_usb_config(self, device_path: str):
        """Create USB serial configuration."""
        from ._first_run_wizard import create_usb_config
        create_usb_config(self, device_path)

    def _wizard_step_hardware(self):
        """Wizard Step 1: Hardware Detection"""
        from ._first_run_wizard import wizard_step_hardware
        wizard_step_hardware(self)

    def _is_raspberry_pi(self) -> bool:
        """Check if running on Raspberry Pi."""
        from ._first_run_wizard import is_raspberry_pi
        return is_raspberry_pi(self)

    def _offer_enable_spi(self) -> bool:
        """Offer to enable SPI on Raspberry Pi. Returns True if enabled."""
        from ._first_run_wizard import offer_enable_spi
        return offer_enable_spi(self)

    def _wizard_step_services(self):
        """Wizard Step 2: Service Status"""
        from ._first_run_wizard import wizard_step_services
        wizard_step_services(self)

    def _wizard_step_config(self):
        """Wizard Step 3: Quick Configuration"""
        from ._first_run_wizard import wizard_step_config
        wizard_step_config(self)

    def _wizard_complete(self):
        """Wizard completion"""
        from ._first_run_wizard import wizard_complete
        wizard_complete(self)

    def _settings_run_wizard(self):
        """Run wizard from settings menu."""
        from ._first_run_wizard import settings_run_wizard
        settings_run_wizard(self)
