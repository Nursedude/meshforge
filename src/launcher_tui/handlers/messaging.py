"""
Messaging Handler — Send/receive messages, view history, conversations.

Converted from messaging_mixin.py as part of the mixin-to-registry migration.
"""
...
class MessagingHandler(BaseHandler):
    """TUI handler for messaging operations."""
    ...
    def _messaging_send(self):
        ...
        text = self.ctx.dialog.inputbox("Message", "Enter message text:")
        if not text:
            return
        ...
        if is_running:
            ...
            if action:
                ...
                if result.success:
                    ...
                    self.ctx.dialog.msgbox("RX Failed", f"{result.message}\n\n{result.error or ''}")
