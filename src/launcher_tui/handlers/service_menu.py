...
class ServiceMenuHandler(BaseHandler):
    ...
    def _install_native_meshtasticd(self):
        ...
        self.ctx.dialog.infobox("Installing", "Installing native meshtasticd...")

        try:
            result = subprocess.run(['which', 'meshtasticd'], capture_output=True, text=True, timeout=5)
            if result.returncode != 0:
                self.ctx.dialog.infobox("Installing", "Adding Meshtastic repository...")
                ...
                self.ctx.dialog.infobox("Installing", "Updating package list...")
                subprocess.run(['apt-get', 'update'], timeout=120, check=False)

                self.ctx.dialog.infobox("Installing", "Installing meshtasticd...")
                result = subprocess.run(['apt-get', 'install', '-y', 'meshtasticd'], timeout=300, capture_output=True, text=True)

                if result.returncode != 0:
                    self.ctx.dialog.msgbox("Error", f"Failed to install meshtasticd:\n{result.stderr[:500]}")
                    return
            ...
            if config_yaml.exists():
                self.ctx.dialog.infobox("Installing", "Config structure ready")

            usb_config = config_dir / 'config.d' / 'usb-serial.yaml'
            if usb_config.exists():
                usb_config.unlink()
                self.ctx.dialog.infobox("Installing", "Removed incorrect USB config")
...
"""
            ...
            if not success:
                self.ctx.dialog.msgbox("Warning", f"Service setup issue: {msg}")
    ...
    def _configure_openhamclock_via_settings(self):
        ...
        settings = self.ctx.registry.get_handler("settings")
        if settings:
            settings._configure_openhamclock()
        else:
            self.ctx.dialog.msgbox("Not Available", "Settings handler not loaded.")
    ...
    def _mqtt_setup_wizard(self):
        ...
        self.ctx.dialog.infobox("MQTT Setup", "Step 1/3: Checking mosquitto...")
        ...
        if not self._is_mosquitto_installed():
            ...
            self.ctx.dialog.infobox("MQTT Setup", "Mosquitto is already installed.")

        self.ctx.dialog.infobox("MQTT Setup", "Step 2/3: Starting mosquitto service...")
