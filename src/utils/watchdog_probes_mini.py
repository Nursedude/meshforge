...
# Dreams reader for the value-loop probe. Same guarded-import discipline as
# _calib — read the deltas via the OWNING module so probe and dream pass can
# never disagree on how a proposal is loaded (honest_failure_modes #5).
try:
    from mini_dudeai import dreams as _dreams
except Exception:  # mini package absent in some contexts
    _dreams = None

_CALIB_LEDGER_NAME = "calibration_ledger.jsonl"
_MINI_DELTAS_NAME = "mini_dudeai_memory_deltas.jsonl"
# SSOT: mini_dudeai.claw_telemetry.CADENCE_VERDICT_NAME. Kept as a local literal
# (claw_telemetry pulls a heavier import graph) and TEST-PINNED byte-identical,
# the same layering compromise as _seed_rules_path (honest_failure_modes #5).
_CADENCE_VERDICT_NAME = "mini_cadence"

# dream_ratification_stalled tunables — conservative to stay HIGH-PRECISION (the
# whole point of the value loop is to REDUCE noise; this probe must add none).
# A proposal unresolved past MIN_AGE has survived ~2 of the 7-day resolve-suppress
# windows without being ratified OR rejected. Env-overridable.
try:
    DREAM_STALL_MIN_KEYS = int(
        os.environ.get("MESHFORGE_DREAM_STALL_MIN_KEYS", "5"))
except (TypeError, ValueError):
    DREAM_STALL_MIN_KEYS = 5
try:
    DREAM_STALL_MIN_AGE_S = float(
        os.environ.get("MESHFORGE_DREAM_STALL_MIN_AGE_DAYS", "14")) * 86400.0
except (TypeError, ValueError):
    DREAM_STALL_MIN_AGE_S = 14 * 86400.0
# A mini_cadence verdict older than this = the cadence loop is not currently
...
def _read_cron_verdicts_text(home: Optional[str]) -> Optional[str]:
    """Read ``~/cron_verdicts.log`` root-safe. None on absent/unreadable (a
    POSITIVE empty read is ``""``, distinct from an unreadable log)."""
    if not home:
        return None
    try:
...
def probe_dream_ratification_stalled(
    *,
    ...
    now_ts: Optional[float] = None,
    min_keys: int = DREAM_STALL_MIN_KEYS,
    min_age_s: float = DREAM_STALL_MIN_AGE_S,
    cadence_alive_max_age_s: float = DREAM_STALL_CADENCE_ALIVE_MAX_AGE_S,
) -> Optional[Signal]:
    """Fire when mini's dream propose->ratify VALUE loop RUNS but does not CLOSE.

    The nightly dream pass proposes memory-deltas; a cadence session is meant to
    ratify or reject each. The 2026-07-18 sweep found 0/164 ratified over 7 weeks
    and mini had no idea. This routes "proposals pile up unresolved while the
    ratifier is demonstrably alive" to the fleet surface.

    NON-redundant with cron_verdict_stale (#78) BY DESIGN: that owns the
    cadence-DOWN case (a failing/stale ``mini_cadence`` verdict); this fires ONLY
    when the cadence verdict is fresh + OK (the loop is up) yet the backlog is not
    clearing. The cadence-liveness gate also SCOPES the probe to the manager box
    for free — a box with no ``mini_cadence`` verdict has no ratifier, so an
    unresolved backlog there is EXPECTED, not a stall (INERT).

    Self-guards: mini unimportable -> inert; operator home unresolvable ->
    indeterminate; no deltas file (box runs no dream loop) -> inert;
    cron_verdicts.log unreadable -> indeterminate (liveness unobservable, never a
    false clean); no/failing/stale cadence verdict -> inert (deferred to #78).
    degraded only; the seed routes it escalate-only (NO ntfy page) until it soaks
    low-false-positive, the calibration_drift precedent. Never raises into the
    tick."""
    ...
    try:
        ...
        if _dreams is None:
            note_disposition(cls, "inert", reason="mini_dudeai unimportable")
            return None

        home = None
        if deltas is None or verdicts_text is None:
            home = _resolve_mini_home()

        # --- deltas backlog source ---
        if deltas is None:
            if deltas_path is None:
                if not home:
                    note_disposition(cls, "indeterminate",
                                     reason="operator home unresolvable")
                    return None
                deltas_path = os.path.join(home, _MINI_DELTAS_NAME)
        ...
        #     double-firing with cron_verdict_stale (#78) ---
        if verdicts_text is None:
            verdicts_text = _read_cron_verdicts_text(home)
        if verdicts_text is None:
            note_disposition(
                cls, "indeterminate",
        ...
        # --- backlog measure: keys whose LATEST status is 'proposed', and the
        #     oldest FIRST-proposal age. An unresolved key is single-row (it is in
        #     dreams._skip_keys, never re-proposed), so its ts is the original. ---
        latest: dict = {}
        first_proposed: dict = {}
        for d in deltas:
            ...
            if d.get("status") == "proposed":
                ...
                if isinstance(ts, (int, float)):
                    prev = first_proposed.get(k)
                    first_proposed[k] = ts if prev is None else min(prev, ts)
        unresolved = [k for k, d in latest.items()
                      if d.get("status") == "proposed"]
        ...
        cad_hrs = int(age // 3600)
        return Signal(
            # Literal (not the `cls` var) so the run_all_probes reachability gate
            # (test_honesty_invariants) statically sees this class is emitted.
            cls="dream_ratification_stalled",
            subject="dream-loop",
            severity="degraded",
            detail=(
                f"{n} dream proposal(s) unresolved (oldest {oldest_days}d) while "
                f"mini_cadence is alive (verdict {status}, {cad_hrs}h ago) — the "
                f"propose->ratify loop runs but is not closing. Review: "
                f"`python3 -m mini_dudeai.dreams --list-proposed`, then ratify or "
                f"reject each with a --reason (noise => retune the detector). The "
                f"2026-07-18 sweep found 0/164 ratified and could not say why."
            ),
            extra={"unresolved": n, "oldest_age_s": round(oldest, 0),
                   "cadence_status": status, "cadence_age_s": round(age, 0),
                   "min_keys": min_keys, "min_age_s": min_age_s},
        )
    except Exception:
