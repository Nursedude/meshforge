...
# resolve_probe_targets moved to utils.watchdog_retarget (MF025 split,
# 2026-08-27) -- re-exported here so callers and existing test patch-paths
# keep working; it imports this module's _role_expected_active alias at
# call time, so patching utils.watchdog_runner._role_expected_active
# still takes effect.
from utils.watchdog_retarget import resolve_probe_targets  # noqa: E402,F401
...
def run_loop(
    *,
    ...
    # Re-read probe targets when a declaration file changes (mtime-gated —
    # rationale + cost argument in utils.watchdog_retarget).
    decl_gate = None
    if retarget is not None:
        from utils.watchdog_retarget import make_declaration_gate
        decl_gate = make_declaration_gate()

    while not stop_event.is_set():
        now = time.time()
        probe_count += 1
        if decl_gate is not None:
            moved = decl_gate.changed()
            if moved:
                try:
                    services_expected_active, services_wedge_check = retarget()
                    logger.info(
                        "watchdog: %s changed — probe targets re-resolved "
                        "(%d expected-active unit(s))",
                        moved, len(services_expected_active))
                except Exception as exc:
                    # Keep the previous lists: a failed re-read must not
                    # narrow or widen coverage on a guess (hfm #2).
                    logger.error(
                        "watchdog: probe-target re-resolve failed after %s "
                        "changed — keeping previous targets: %s", moved, exc)
        try:
...
def main(argv=None) -> int:
    ...
    try:
        run_loop(
            ...
            # Declaration changes re-resolve probe targets mid-run
            # (mtime-gated in run_loop; utils.watchdog_retarget).
            retarget=lambda: resolve_probe_targets(
                load_config_file(args.config))[:2],
        )
    except Exception as exc:
