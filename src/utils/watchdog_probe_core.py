...
SIGNAL_CLASSES = (
    ...
    "json_uplink_dark",  # 2026-08-07: a box DECLARING organ_expectations.json_uplink is producing NO MQTT-json uplink (zero 'serialized json message' lines, or a newest one older than the darkness threshold) while its meshtasticd journal demonstrably covers the window. The instrument-liveness half of channel_feed_dark, which watches the mesh THROUGH this uplink and is therefore blind without it. Born from the 2026-08-07 dig: kiai/moc1/moc4/moc5 all sat `indeterminate` on channel_feed_dark with reason "mqtt off or journalctl error" while journalctl was returning rc=1 (ran, positively no match) against healthy 12k-15k-line journals — and 3 of the 4 had mqtt.enabled=True, so the stated reason was wrong, not merely vague. The real gate was mqtt.json_enabled, a DIFFERENT setting, False on exactly those four and True on exactly the three that worked (7/7, no exceptions). Fixing that to `inert` alone would have been the same defect in a new skin: "zero json lines -> inert" ALSO silences a box whose uplink genuinely died more than the lookback ago (the freshness gate only catches a death INSIDE the window), turning this probe's own failure mode — silence looks like no traffic — on its own instrument. So absence is benign only where the box never claimed the organ; the declaration separates off-by-design from died-quietly, and declared-and-absent is this class. Journal-only by construction: reading mqtt.json_enabled from the radio would open a TCPInterface and steal a PhoneAPI slot (#17/MF007). Guarded by a journal-coverage check so a fresh boot or a vacuumed/volatile journal is indeterminate, never "dark" — and note journalctl exits 0 with EMPTY stdout when nothing is old enough, so the LINE is the evidence, not the rc. degraded only; INERT on every undeclared box; unreadable declaration is indeterminate. Documented inline (MF012 40k cap; same precedent as meshtasticd_vsz_leak). No own issue#.
