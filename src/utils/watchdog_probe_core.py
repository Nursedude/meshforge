...
SIGNAL_CLASSES = (
    ...
    "mini_watchdog_source_unwired",  # 2026-09-03: this box RUNS a MeshForge watchdog (producing /var/lib/meshforge/watchdog.json) while the mini process actually running here was started with MINI_DUDEAI_ENABLE_WATCHDOG=0 — its signal_class feed is switched OFF, so every signal this watchdog emits lands in a reader that is not listening. mini keeps ticking at src_errors=0 and reads GREEN, indistinguishable from a box that is genuinely healthy: the silent half of honest_failure_modes #4 (reader/writer pairs wire together or fail together). The LOUD half is already covered — ENABLE_WATCHDOG=1 with no watchdog emits source_error_watchdog and pins src_errors=1 every tick — so this closes the polarity that never complains (the frozen-green lesson: fix both directions together). The flag is legitimate and REQUIRED on a box that runs no watchdog (lehua, the MeshAnchor box), which is why it cannot simply be removed; the defect is the COMBINATION, and it appears the day a watchdog is installed on such a box and nobody flips the flag back — a transition the env file documents in prose and nothing enforced. Reads the RUNNING process's own /proc/<pid>/environ, never the config file: the consumer-of-record, not the wiring (calibrated_claims #7). INERT where no watchdog unit exists here or it is not running (no producer, nothing to contradict); indeterminate when mini is seeded but not running, or /proc is unreadable. Documented inline (MF012 40k cap). No own issue#.
    "mini_rule_orphaned_exclusion",  # 2026-09-03: a LIVE mini rule carries a subject_exclude_globs entry that no OTHER live rule of the same match.kind claims — a deliberate blind spot whose owner is gone. An exclusion exists because some other rule OWNS that subject (the seed says so in prose: "subject_exclude_globs skips moc3 (moc3_federation_backoff_known_normal owns it)"); retire the owner and the exclusion silently becomes an unwatched hole. Born from the moc3 retirement (f36c93c2): merge_seed_rules is strictly ADDITIVE, so retiring a rule from the seed leaves the live copy behind, and --prune (0b6a34f6) can only remove copies it can prove UNMODIFIED — a box-TUNED rule carrying the exclusion is kept verbatim BY DESIGN, which is exactly where a retirement half-lands. The federator box hit that: rule gone, glob still blinding that vantage with nothing owning it. The seed-side twin is guarded by tests; nothing guarded LIVE box copies, which is the half that actually runs. Ownership is decided by the ENGINE's own matcher (mini_dudeai.engine._match_rule — kind, one selector, exclusions, extras filters such as `class`) over subjects the box has actually recorded in mini's state.json, falling back to the glob's literal core as a synthetic subject only when no observed subject is in scope; a glob nobody can judge reads indeterminate and is named (2026-09-03 frontier pass: the first cut's private glob-core solver read holes as owned in two shapes and paged on owned subjects in three) — so the documented box-local pattern (a local *_known_normal rule + its matching exclude, MF014 operator-specific and deliberately absent from the repo seed) reads CLEAN. INERT where no live rules file exists; indeterminate when it is present but unreadable, or when every exclusion is a bare "*" nothing can reason about. Documented inline (MF012 40k cap). No own issue#.
    ...
    "user_timer_unit_failing",  # 2026-07-19: an enabled USER *timer*'s job fails on EVERY firing (>=N "Failed with result" in a short window, newest fresh, and no success since) — the last uncovered corner of the user-unit blindness class. probe_service_inactive is blind to user units; nomadnet_crashloop watches one unit's LIVE restart loop; user_unit_inactive judges only always-on default.target.wants daemons via invocation markers and EXPLICITLY excludes timers (no invocation marker, and a oneshot is inactive between firings by design). Found the hard way: kiai's meshforge-tracer.timer fired every 10 min from 2026-07-12 while its oneshot exited 2 ("no peers in lab_peers") every time — a week silent, every existing leg reading it healthy. Bus-free root-readable USER_UNIT= journal read; recency gate so a fixed job stops paging; per-unit unobservable is skipped and all-unobservable holds the streak (journalctl wedge != all healthy); 2-tick debounce. CADENCE-AWARE since 2026-08-13: each timer's window and recency gate are sized from its OWN schedule (utils.timer_cadence asks systemd-analyze), closing the 2026-07-21 W4 residual where a timer slower than ~90 min could fail every firing inside the flat 3h window and never reach min_failures — measured that day as 13 of 28 enrolled user timers fleet-wide, including every box's dream timer. The residual MOVED rather than vanished: a cadence-sized window can outrun the journal meant to fill it (fleet horizons 28h–102h, and ~10 min on meshanchor-server), so a timer is judged ONLY when the window witnessed >= min_failures firing OUTCOMES, and the disposition NAMES the ones it could not judge instead of folding them into an affirmative clean. No systemd-analyze -> cadence None -> the flat floor, i.e. exactly the pre-2026-08-13 behaviour, warned once. Documented inline (MF012 40k cap precedent); no own issue#.
    ...
    "rf_leg_silent",               # 2026-09-08: an RNode RF interface is transmitting while receiving NOTHING, sustained, on a leg that has received before. Born self-inflicted: a capture tool left two production RNodes in PROMISCUOUS mode and exited; promiscuous is a MODE the device stays in and RNS's own RNodeInterface init never clears it, so both radios went deaf and stayed deaf. Every surface still read healthy — Status: Up, service active, box reachable, traffic flowing — because traffic was flowing over TCP. A redundant path masked a dead one for hours, and it presented as a one-way RF link (transmit worked, so the far end's downlink moved and nothing came back), costing an afternoon on noise floors and a suspected PA receive fault. "Status: Up" is PRESENCE; this probe asks for FUNCTION. ⚠️ The discriminator is the whole design: tx>0 with rx==0 is the NORMAL state of a box whose RF network has no other node — the fleet's only RNode sat at 1.15 MB up / 0 B down for months, correctly — so this fires ONLY where a peer has been heard at least once, then TX grows while RX stays exactly flat, sustained over 3 ticks. Counters going backwards is an rnsd restart, not deafness: the streak resets. inert where no RNode leg exists (most boxes) or none has ever received; indeterminate when rnstatus is unavailable. Matters most on a PURE-RF gateway, where the identical fault is a total outage rather than an invisible one. Cured by lint MF029 + utils.rnode_session; this is the detection half. No own issue#.
