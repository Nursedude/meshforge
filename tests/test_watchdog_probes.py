...
def test_signal_classes_closed_enum_is_documented():
    ...
    assert set(SIGNAL_CLASSES) == {
        ...
        "mini_watchdog_source_unwired",  # 2026-09-03 — this box runs meshforge-watchdog while its LIVE mini process was started with MINI_DUDEAI_ENABLE_WATCHDOG=0: the watchdog's signals land in a reader that is not listening, and mini ticks green at src_errors=0. The flag is REQUIRED on a watchdog-less box (it prevents a permanent source_error_watchdog), so the defect is the COMBINATION — it appears when a watchdog is later installed and nobody flips the flag back, a transition lehua's env file documents in prose and nothing enforced. The loud polarity already pins src_errors; this closes the silent one (honest_failure_modes #4, both directions). Reads /proc/<pid>/environ, the consumer-of-record, never the config file. Documented inline in the SIGNAL_CLASSES comment (MF012 40k cap). No own issue#.
        "mini_rule_orphaned_exclusion",  # 2026-09-03 — a LIVE mini rule excludes a subject no other same-kind rule watches: a deliberate blind spot whose owner was retired. merge_seed_rules is strictly additive and promote_seed_rules --prune keeps box-TUNED rules verbatim BY DESIGN, so a retirement lands fully on boxes running an unmodified seed copy and HALF on any box that tuned it (measured on the federator during the moc3 retirement, f36c93c2). Seed-side halves are test-guarded; this watches the LIVE box copies, the half that actually runs. Ownership decided by the engine's own matcher over observed subjects (structural core only as fallback; unjudgeable named), so the documented MF014 box-local *_known_normal + matching-exclude pattern reads clean. Documented inline in the SIGNAL_CLASSES comment (MF012 40k cap). No own issue#.
        ...
        "rf_leg_silent",                 # 2026-09-08 — an RNode RF leg TRANSMITTING while receiving nothing, sustained, on a leg that has received before. "Status: Up" is PRESENCE; this asks for FUNCTION. Born self-inflicted: a capture tool left two production RNodes in promiscuous mode and exited — promiscuous is a MODE the device keeps and RNS's RNodeInterface init never clears it, so both radios went deaf while every surface read healthy (traffic was still flowing over TCP — a redundant path masking a dead one). Discriminator is the whole design: tx>0/rx==0 is the CORRECT steady state of a box whose RF network has no other node, so this fires only where a peer was heard at least once, then TX grows while RX stays exactly flat over 3 ticks; counters going backwards is an rnsd restart, not deafness. inert where no RNode leg exists or none has ever received; indeterminate when rnstatus is unavailable. Cured by lint MF029 + utils.rnode_session; this is the detection half. No own issue#.
