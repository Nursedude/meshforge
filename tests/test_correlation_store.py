...
class TestLookupDirecter:
    """The r2m reverse lookup — durable backstop for bot round-trips.

    When an RNS client directs at a mesh node (@!id -> bot), an r2m row
    is recorded; lookup_directer reads it back so the node's reply routes
    HOME even when the in-memory session is gone (restart/expiry).
    """

    def test_record_lookup_roundtrip(self, store):
        assert store.record(MESH_A, PEER_A, direction="r2m") is True
        assert store.lookup_directer(MESH_A) == PEER_A

    def test_lookup_missing_returns_none(self, store):
        assert store.lookup_directer(MESH_A) is None

    def test_m2r_rows_do_not_leak(self, store):
        # An m2r row (mesh messaged peer) must NOT answer a directer lookup.
        assert store.record(MESH_A, PEER_A, direction="m2r") is True
        assert store.lookup_directer(MESH_A) is None
    ...
    def test_restart_survives(self, tmp_path):
        path = str(tmp_path / "corr.db")
        s1 = BridgeCorrelationStore(db_path=path)
        assert s1.record(MESH_A, PEER_A, direction="r2m") is True
        s2 = BridgeCorrelationStore(db_path=path)
        assert s2.lookup_directer(MESH_A) == PEER_A

    def test_latest_r2m_wins(self, store, clock):
        store.record(MESH_A, PEER_A, direction="r2m")
        clock[0] += 10
        store.record(MESH_A, PEER_B, direction="r2m")  # B directed at A last
        assert store.lookup_directer(MESH_A) == PEER_B

    def test_expired_directer_returns_none(self, store, clock):
        store.record(MESH_A, PEER_A, direction="r2m")
        clock[0] += DEFAULT_TTL_SEC + 1
        assert store.lookup_directer(MESH_A) is None
