...
class TestCorsOriginHardening:
    ...
    preserving the /24 intent of the `http://192.0.2.` prefix."""

    ALLOWED = ['http://localhost', 'http://127.0.0.1', 'http://192.0.2.']
    ...
    def test_lan_slash24_octet_allowed(self):
        from utils.map_http_handler import _origin_allowed
        assert _origin_allowed('http://192.0.2.41', self.ALLOWED)
        assert _origin_allowed('http://192.0.2.41:5000', self.ALLOWED)

    def test_subdomain_suffix_bypass_rejected(self):
        from utils.map_http_handler import _origin_allowed
        # These all passed the old startswith() check.
        assert not _origin_allowed('http://192.0.2.evil.com', self.ALLOWED)
        assert not _origin_allowed('http://192.0.2.41.evil.com', self.ALLOWED)
    ...
    def test_scheme_mismatch_and_empty_rejected(self):
        from utils.map_http_handler import _origin_allowed
        assert not _origin_allowed('https://192.0.2.41', self.ALLOWED)
...
class TestClientTrustGate:
    ...
    def test_configured_lan_trusted(self):
        from utils.map_http_handler import _client_ip_trusted
        assert _client_ip_trusted('192.0.2.99', self.ALLOWED)
    ...
    def test_no_cors_configured_means_loopback_only(self):
        ...
        assert not _client_ip_trusted('192.0.2.99', None)
