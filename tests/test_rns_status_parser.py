...
FULL_OUTPUT = """\
 Shared Instance[rns/meshforge fleet-host-2 rns]
    Status    : Up
    Serving   : 0 programs
    Rate      : 1.00 Gbps
    Traffic   : ↑242 B  0 bps
...
class TestParseDisplayNames:
    def test_shared_instance_name(self):
        result = parse_rnstatus(FULL_OUTPUT)
        assert result.interfaces[0].display_name == "rns/meshforge fleet-host-2 rns"
    ...
    def test_full_name_property(self):
        result = parse_rnstatus(FULL_OUTPUT)
        assert result.interfaces[2].full_name == "TCPInterface[Regional RNS/192.0.2.38:4242]"
