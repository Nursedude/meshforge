...
**Four residuals the metric could not see** (1–3 surfaced by the fleet's
own detectors within hours, all closed 08-11, runbook commit `c1199370`;
#4 went unrecorded for a day and is the only one no detector could raise,
because the detector WAS the defect):
...
4. **The drill made the fleet lie about a box it never touched** (found
   2026-08-11, one day late, by reading the warm brief rather than by any
   detector). kiai reaches this manager ONLY through the reverse tunnel
   alaula pins, so factory-resetting alaula took kiai's only path with it.
   `fleet_offline_check.sh` read that ssh failure as box state and paged
   **"Fleet box DOWN: kiai"** for 54 min (16:01→~16:56, page #1) about a box
   that is **up 17 days and never rebooted**. Nothing in the drill record
   mentions the page; the runbook noted the tunnel loss only as an *access*
   problem for alaula (finding #4 there), so the fleet-side consequence had
   no owner. **This is a false page of exactly the kind T3 pre-registers as
   a failure condition** — "nothing may page falsely on the fleet side" —
   generated by T1, a month before T3 will test for it, and scored PASS.
   Cured 2026-08-11 (not suppressed — a drill-window mute would go dark on
   real kiai outages): a box may declare its dependency path in the monitor's
   box config, and an unreachable box whose path is ALSO down is paged as
   **UNOBSERVABLE — state UNKNOWN**, never DOWN. The verdict rides state
   field 7 so mini/watchdog word it identically. Guard-drilled in both
   directions (fail-to-demote and blanket-excuse) per the plant-a-violation
   rule; 24 + 18 tests green.

**Method note this residual argues for:** #1–#3 were caught by detectors
because the drill changed the *subject* box. #4 was invisible because the
drill changed the subject box's **neighbours' view of an unrelated box** —
blast radius nobody had enumerated. Future destructive trials should record,
before the destructive step, *which other boxes lose their only path to what*
— and treat every page raised during the window as a finding to be explained,
not as noise to be waited out. (The 08-10 pages were seen and correctly
attributed in the moment; what was missing was writing down that the monitor
had made a claim it could not support.)
