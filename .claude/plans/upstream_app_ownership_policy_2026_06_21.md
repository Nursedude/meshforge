...
> **Status:** IN PROGRESS (2026-06-21) — inventory complete, policy + decisions approved. **DONE:** Action 1 (rescue → version control); Action 3 (moc5 meshchat converged onto our fork — provenance, no restart); Action 4 (bot tier = T2, fork BUILT, VolcanoAI canary green). **GATED post-06-24:** the `meshing-around` `.32` deploy + the `meshchat` functional modernization. **Remain:** Action 2 (pin); Action 5 (drift check, blocked on clean trees).
