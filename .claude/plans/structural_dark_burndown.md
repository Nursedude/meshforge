...
| 8 | ~~`cross_gateway_dups_unsuppressed`~~ | **ACCEPTED-PERMANENT 2026-07-19 (operator): keep the detector, do NOT build coordination.** Reason = COST ASYMMETRY (a dup is redundancy; a yield-protocol bug is SILENCE — on emergency comms, fail toward redundancy), NOT a low rate: 3 HUMAN recipients are dual-homed TODAY, so the precondition is live and time WILL move the rate. Residual/leading indicator: dual-homed-recipient COUNT, observable even on the mesh leg. See the addendum in `.claude/research/cross_gateway_dup_design_inputs_2026_07_19.md`. **RATIFIED AT FRONTIER TIER 2026-07-26** (Fable 5): four attacks mounted incl. static recipient-ownership — refuted by the fleet's dominant partial-failure class (#68/#72, heartbeat ≠ delivery capability). Verdict + revisit triggers (first levers are topology, never a protocol) in the same file's VERDICT section; eval `oracle-cross-gateway-dup-accept-cost-asymmetry`. Leading indicator SHIPPED (`dual_homed_recipients` in rollup + `probe_gateway_dual_homed_exposure` edge-fire; live value 3). ROW CLOSED. | — | — | ~~next slice = surface `dual_homed_recipients` (map layer, NOT roll-blocked)~~ shipped |
