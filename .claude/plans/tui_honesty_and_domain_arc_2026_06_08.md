...
- [x] **L1–L5** DONE (MeshAnchor `6311722b`; L3 also MeshForge `1834fcc` — shared `active_health_probe.py`). **L1** documented the necessary user-unit exception (`check_service` is system-scope only — answered: no `--user` support). **L2** reviewed, no-op (best-effort confirmation DM; `added` state real; H2 owns delivery). **L3** `get_status` emits `uptime_percent=None` (not 0.0) for never-checked. **L4** `_collect_daemon_federation_peers` returns `(peers, err)` → rollup threads the registry-fetch failure into `rollup.errors` (no silent "0 peers"). **L5** `validate_meshcore_device` `responds=None` (not fabricated True) when a persistent owner holds the radio. Guards `TestLowTierS8` (MA) + `TestUptimePercentSurfaceL3` (MF) + updated `test_meshcore_connection`. L4 **LIVE** — `meshanchor-map` restarted 2026-06-08 (clean, healthz 200; `/fleet/rollup` 50 federation_peers, 0 spurious errors). **S8 COMPLETE.**
- [x] **Broader `.save()` sweep (deferred S5) — DONE** (MeshForge `9f24ebd` + MeshAnchor `53e2799f`, both — byte-identical handlers). `SettingsManager.save()` bool was discarded at 10 sites; audited each per the S5 note: **6 false-success** (automation auto-ping/traceroute/auto-discover/welcome toggles + edit-targets, first_run callsign) → `report_action(settings.save(), …)`; **4 benign silent-saves** (intervals/message/cooldown) → new `_save_or_warn()` (failure-only surface). Guard: extended `TestSaveReturnChecked` to `(cfg|config|settings).save()` handler-wide (the full sweep — cfg.save S5 + meshcore config.save S8 M1/M2 + this — is complete, so all three idioms are guarded against a bare discard). Lint 0, MF 52 / MA 61 guard+regression pass, parity in sync.

> **🏁 ENTIRE HONEST-SIGNAL ARC CLOSED (2026-06-08).** Thread 1 (S0–S7, 31 defects) + the MA-divergent audit (H1–H2 / M1–M4 / L1–L5, 11 findings) + the broader `.save()` sweep. Both repos, fleet + meshanchor-server deployed, parity in sync throughout — **all daemon-latent fixes (M3, L4) are now LIVE** (meshanchor-map restarted 2026-06-08, verified). The TUI honest-signal contract — *it works, or it says exactly how it didn't* — now holds across both apps with a regression-guard suite (`test_honest_signal_guards.py` + the extended `test_regression_guards.py` checks) that ports between the diverged repos. **Remaining in this plan: Thread 2 (bidirectional addressability feature) + Thread 3 (fleet bootstrap)** — no honest-signal items left.
...
- [x] **Step 4 — consume Meshtastic `ROUTING_APP` ACK/NAK for honest delivery
  confirmation (Thread-2 Phase 2, default-off) — WORKS IN BOTH BRIDGE MODES.** TCP
  mode: the ACK rides the persistent `meshtastic.receive` (`MeshtasticHandler`).
  `mqtt_bridge` mode (fleet default): the soak (2026-06-08) caught the TCP wiring
  INERT here (`81ac4ac` honest-signal), then the `/e/` ServiceEnvelope path was
  BUILT (`204da9e`+`3fc9321`): `src/utils/meshtastic_se_crypto.py` native AES-CTR
  decrypt (proven live vs `/json/`), `send_text_direct_with_id`, `MQTTBridgeHandler`
  decodes ROUTING_APP from `/e/` (cost-guarded, NO fromradio read → #17/#75 held),
  `meshtastic.channel_keys` config. Live CONFIRMED awaits a reachable node + the
  downlink channel's PSK in `channel_keys` (custom channels; default = zero-config).
  Research: `.claude/research/mqtt_bridge_ack_feasibility_2026_06_08.md`. New
  `src/gateway/ack_tracker.py`:
...
- [~] **Config deltas — assertion-upgrade DONE (`b84023f`, MeshForge-only).** ⚠️ **GROUND-TRUTH finding that reshaped this task:** the map `defaults` are NOT operator-settable — `response_caches` are unconditional in `MapDataCollector` (#70/#71), `node_cap` is the `DEFAULT_DIRECTORY_MAX_ROWS` code constant (#49/#50), `bbox` auto-derives from the operator position (deployment-specific), and `want_ack` is MeshAnchor-role-only (skipped). So there was *nothing to mutate* — "enforce by setting" doesn't apply (the safe defaults are baked into code by construction, the "software fits the box" invariant). New `config_delta_actions()` (parallel to `foundation_actions()`, MAP roles only) **asserts** them against real state, **read-only even under `--apply`** (emits only noop/warn): `delta:node-cap` cross-checks the code constant vs the yaml (catches code↔declaration drift — the real reproducibility hazard), `delta:response-caches` asserts the layer present, `delta:bbox-anchor` warns if no operator position/explicit bbox (filter OFF → unfiltered firehose). Live-verified across the role matrix (map boxes PASS, gateway-only emits none). 7 tests. **The one genuinely force-settable delta — meshtasticd `mqtt.root` (#77) — is the real mutation-enforcement target, a separate radio-touching slice (PhoneAPI/#17, not low-risk).** foundation perms already enforced (`foundation_actions`).
- [x] **v3 loop — DONE (`6f0d4b2`, MeshForge-only, GATED OFF).** Watchdog drift → provisioner re-converge. The `role_drift` signal (`probe_role_drift`) + the Phase-2 auto-remediation machinery (7 gates, dry-run-first) already existed; v3 generalized Phase 2 from a single `systemctl restart` to a pluggable action: `AutoRestartRule.action` (`"restart"`|`"reconverge"`), new `execute_reconverge()` runs `provision_role.py --apply` (its non-zero exit on a failed apply — incl. a sandbox-blocked foundation chown — is the honest success signal), runner dispatches on `d.rule.action`; all existing gates apply. Ships **inert** (no box has a reconverge rule → zero runtime change); an operator opts in per-box in `watchdog.json` dry-run-first (schema example: `cooldown_s=1800, max=1/h`, since it re-applies the whole role). 8 tests. **LIVE-PROVEN end-to-end 2026-06-08 on moc1** (cloud-publisher): added a dry-run reconverge rule to its `watchdog.json`, induced a reversible drift (`systemctl disable meshforge-maps`, process stays up), and the watchdog logged `PHASE 2 DRY-RUN action=reconverge target=cloud-publisher reason=role_drift/cloud-publisher persisted 5 ticks` — without executing. moc1 restored, continues soaking the rule.
  - ⚠️ **CRITICAL BUG the fire-test surfaced + fixed (`5bf7334`):** the `role_drift` dry-run *didn't* fire at first — `probe_role_drift` (AND `probe_parity_drift`) had been **silently dead on the Python-3.13 fleet**. The audit-organ probes importlib-load their SSOT scripts (`provision_role.py`, `parity_check.py`) via `module_from_spec`+`exec_module` *without* registering the module in `sys.modules` first; on py3.12+ the `@dataclass` field-type eval reads `sys.modules[cls.__module__].__dict__` → `AttributeError`, and the probes' broad `except Exception: return None` swallowed it (returned "indeterminate" forever). Fix = `sys.modules[spec.name] = mod` before exec at all 3 load sites. Regression test exercises the REAL default `plan_fn` (every existing test injected a fake `plan_fn`, which is exactly why it slipped). Fleet watchdogs restarted (incl. VolcanoAI); verify-live = no role_drift/parity_drift firing anywhere (all converged + in-sync).
