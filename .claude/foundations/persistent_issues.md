...
| a loss/rate detector fires constantly at EXACTLY its threshold value, and its lower severity rung has NEVER once been seen | the alarm bar is sitting ON the SAMPLING FLOOR (2026-09-15) — `wan_path` pinged `PING_COUNT=20` with `LOSS_FAIL_PCT=5.0`, so the smallest observable loss (1 packet = 5%) WAS the fail bar and `concern` was unreachable by construction: 1008 samples/7d = 33 FAIL, **0 concern**, on far loss of 0.00–0.20%/day (10 packets of 8,640 — a clean link). **Quick check**: are the recorded values all multiples of `100/N`? then N is your resolution and any threshold ≤ it is a one-event hair trigger. ⚠️ the dead rung had a PASSING test — it fed 2.0%, which 20 packets cannot produce. ⚠️ severity also COLLAPSES: 20 pings per 600 s is a **3.3% duty cycle**, so a total WAN blackout (the 03:02 Starlink firmware reboot) rendered as the same one-packet shape it alarmed on. Cured `dfb299dc`: fail 10%/concern 5%, `unclean_streak` derived from the EXISTING history (not a new streak file — that class freezes below threshold when the dir is unwritable), persistence asked of `concern` only, never `fail`/`unknown`. ⚠️ it had a TWIN one grep away — `path_trace.py` `LOSS_FLOOR_PCT=10.0` vs `DEFAULT_PROBES=10`, where the hop walk then blamed a NAMED router on one dropped probe (`1e69df92`). Fix a threshold, grep for its copies |
| a MeshCore channel filter refuses EVERYTHING, or channel-based logic never matches | **the channel is a NAME in the TEXT, not an index** (2026-09-18, self-inflicted). MeshAnchor delivers `<channel> <sender>: <text>` (`_parse_meshcore_channel_text`) and `source_address` arrives EMPTY, so `payload['channel']` is never set inbound and `from_meshcore`'s `.get('channel', 0)` makes EVERY channel message read as Public slot 0. An index-based inbound allowlist shipped to the live gateway → `event=3 (100%) channel_suppressed=3`, MC bridging dead 11 min, reverted `8e9ac049`. The oracle gets it right: `MESHANCHOR_ORACLE_MESHCORE_CHANNELS=meshanchor` — **NAMES**. ⚠️ 21 tests passed because the FIXTURE fabricated `payload['channel']` — a mock pinning the author's assumption about the wire format, not the wire. ⚠️ `Bridge MC→Mesh: [MC:p4] test` records NO source channel (the prefix is parsed off and discarded), so Public and private inbound are indistinguishable in the primary log — that is why the leak survived months. **Ship the disclosure before the policy** — done MA `7ef55456`: every inbound bridge now logs `[ch:<name>]`. ⚠️ **STILL LIVE: MeshForge carries the SAME index-based guard on all 9 boxes** (`8c5b5fee`+`8910c156`), inert ONLY because `meshcore.enabled:false` everywhere — it breaks MF MeshCore the instant anyone enables it. REVERT it, do not port the cure: MF has NEITHER header parser (`parse_meshcore_channel_header` / `_parse_meshcore_channel_text` are both MA-only), so its wire format is unverified and assuming symmetry is the same mistake twice. The `ChannelPath` seam itself is sound — keep it. ⚠️ And as of 2026-09-18 only `meshanchor` has EVER been observed in that tag: Public's name is unknown, and whether the header names the SOURCE channel at all is unresolved — three assumptions about this field have been wrong in a row |
