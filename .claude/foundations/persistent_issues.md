...
1. **Mesh → RNS identity surfaces in the LXMF envelope, not the body.**
   ...
   - `fields` dict with namespaced keys so future LXMF clients can parse
     ...
     meshforge_from_long     e.g. "HAT-moc3"
...
- A mesh message from the HAT now shows with subject line
  `HAT-moc3 (!ebfa1b11) via Meshtastic` and body `<just the text>`. The
  conversation is still indexed under the gateway's LXMF source hash
  (`f68c2f56…` on moc3), because that is who LXMF thinks the sender is —
...
**Field-validation note — Issue #37 blocks full end-to-end on moc3.** The
...
- `send_text_direct()` direct-invoked against `localhost:443` (moc3's
  meshtasticd HTTPS port) returned `True` for both broadcast and directed DM
  (`dest=0xebfa1b11` — moc3's own HAT). The downstream path my fix routes to
...
blocked every subsequent request (moc3, 2026-04-22). Throughput was
