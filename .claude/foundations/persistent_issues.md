...
| a probe tells you to check a systemd unit | check that unit EXISTS here FIRST (2026-08-09) — `synth_soak_degraded` said "check meshforge-synth-soak.timer" for 78 days on a box that has never had one; the artifacts were hand-fired lab runs. `systemctl --user status <unit>` → `could not be found` |
