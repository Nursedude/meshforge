...
| a loss/rate detector fires constantly at EXACTLY its threshold value, and its lower severity rung has NEVER once been seen | the alarm bar is sitting ON the SAMPLING FLOOR (2026-09-15) — `wan_path` pinged `PING_COUNT=20` with `LOSS_FAIL_PCT=5.0`, so the smallest observable loss (1 packet = 5%) WAS the fail bar and `concern` was unreachable by construction: 1008 samples/7d = 33 FAIL, **0 concern**, on far loss of 0.00–0.20%/day (10 packets of 8,640 — a clean link). **Quick check**: are the recorded values all multiples of `100/N`? then N is your resolution and any threshold ≤ it is a one-event hair trigger. ⚠️ the dead rung had a PASSING test — it fed 2.0%, which 20 packets cannot produce. ⚠️ severity also COLLAPSES: 20 pings per 600 s is a **3.3% duty cycle**, so a total WAN blackout (the 03:02 Starlink firmware reboot) rendered as the same one-packet shape it alarmed on. Cured `dfb299dc`: fail 10%/concern 5%, `unclean_streak` derived from the EXISTING history (not a new streak file — that class freezes below threshold when the dir is unwritable), persistence asked of `concern` only, never `fail`/`unknown`. ⚠️ it had a TWIN one grep away — `path_trace.py` `LOSS_FLOOR_PCT=10.0` vs `DEFAULT_PROBES=10`, where the hop walk then blamed a NAMED router on one dropped probe (`1e69df92`). Fix a threshold, grep for its copies. ⚠️ **the bar was NOT the whole defect (2026-09-18)**: 80 h later the cure held (FAIL/day 9→1, `concern` reachable at last) yet **4 of the 6 remaining FAILs were ONE far target losing beside two reading 0.0%** — `classify` took `max()` over three INDEPENDENT endpoints (EU VPS, GitHub, Fastly CDN), so one host's blip claimed "loss beyond the ISP". Right quantity, wrong AGGREGATION; raising a threshold moves a hair trigger, it does not remove one. Cured `8c0eaed2`: a path verdict now needs far-target **agreement** — all measured losing = `transit`, some clean = new cause `endpoint` (named, `concern`, persistence-gated, traced), and `wan_autotrace.TRACEABLE_CAUSES` grew `endpoint` in the SAME commit (hfm #7). ⚠️ **the old test PINNED the false fire** — `far=(10.0, 0.0)` asserting FAIL/transit. **Quick check**: does the alarm aggregate INDEPENDENT sources a real fault would hit together? then `max()` is a single-source claim wearing a population's name |
| a MeshCore channel filter refuses EVERYTHING, every channel message reads as slot 0, or a message sent on Public logs as the private channel | **the wire's slot index is `channel_idx`; `from_meshcore` read `channel`, a key the library never sends** (2026-09-18; root cause found by the adversarial review, VERIFIED against the twin's live venv, meshcore_py 2.3.7 `reader.py`: CHANNEL_MSG_RECV sets `channel_idx` + `type` 'CHAN'/'PRIV', never `channel`/`is_channel`/`destination`). So `metadata['channel']` was ALWAYS 0 → the 09-18 index guard refused 100% (`8e9ac049` revert) — and the diagnosis that followed, "the channel is a NAME in the TEXT", was the FOURTH wrong assumption about this field: the text header is the sending node's NAME (`meshanchor p4: wx` from node "meshanchor p4"), which is why two cmds sent on PUBLIC logged `[ch:meshanchor]`. Cured MF `3bcc31b0` / MA `8540d7ec`: read `channel_idx`, `None` when absent (absent is unknown, never Public), ingress INFO line `MeshCore channel rx idx=<n> keys=[…]`, bridge tag `[ch:<idx>]`. ⚠️ **Do NOT revert MF's guard `8c5b5fee`** — an earlier version of this row said to, and named `8910c156`, which IS the ChannelPath seam it said to keep; the guard works once the index is real (inert on the 9 boxes only via `meshcore.enabled:false`). ⚠️ A fixture that fabricates `channel`/`is_channel` pins the author, not the wire — feed `reader.py`'s keys. **Quick check**: the daemon journal's `MeshCore channel rx idx=` line, or `grep -n channel_idx <venv>/meshcore/reader.py`. ⚠️ STILL OPEN: MA's oracle name gate matches the sender-name prefix, so it answers Public queries from any node named "meshanchor …" (owed a move to the index); every MeshCore message arrives `is_broadcast=True` because the reader never sets `destination` (separate finding) |
| a Meshtastic node reads UNHEARD while it is plainly alive | node-cache gap, not radio silence (#4, 2026-07-21) — `to_dict()` wrote `service_type`, `_load_cache()` dropped it; false UNHEARD against a node heard 7x/25h. Three defects were all needed (loader drop, `_merge_node` never refreshing `service_*`, once-recorded name PERMANENT). ⚠️ `rnprobe lxmf.propagation` is NOT a delivery test. Body in the archive |
