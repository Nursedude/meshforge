...
~/.claude/ssh/
  ...
  config               # SSH aliases: moc, fleet-host-1, fleet-host-2, fleet-host-3, pi2w
```
...
| fleet-host-1 | Full deployment + AREDN | Diagnose map issues, restart services, verify |
| fleet-host-2 | Full deployment | Deploy, monitor |
| fleet-host-3 | Behind AREDN router | Diagnose AREDN integration |
...
Claude: ssh fleet-host-1 'git pull'
Claude: ssh fleet-host-1 'find ... -name __pycache__ -exec rm ...'
Claude: ssh fleet-host-1 'systemctl restart meshforge'
Claude: ssh fleet-host-1 'curl localhost:5000/api/status'
Claude: "Deployed. 11,642 nodes now rendering on fleet-host-1:5000."
```
...
7. Deployed to fleet-host-1 via SSH
...
*This white paper was written during the session where the workflow was first demonstrated end-to-end. The fleet-host-1 diagnostic, the cross-repo quality audit, and the public data fallback deployment all happened in a single conversation.*
