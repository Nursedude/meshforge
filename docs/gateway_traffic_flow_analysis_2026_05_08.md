...
1. ~~**Treat `Errno 111` as transient and retry inline.**~~ **Tried 2026-05-08, reverted in `790b6a5`.** Field-tested across four R→M arrivals on moc with 0/100/250 ms backoffs. Wake-up window varied 72-315 ms and grew with each sleep tweak — every refused SYN appears to extend meshtasticd's API-suspend timer rather than waking the listener. Net effect of any positive sleep: the persistent queue's eventual successful retry (~3.4 s later via `_post_toradio`, a different code path that bypasses `send_text_direct`) gets delayed by exactly the sleep amount. No latency improvement; non-zero regression. The defect is not addressable inside `send_text_direct`.
