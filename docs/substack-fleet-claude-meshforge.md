...
**SSH into moc1.** Using a key that was set up in a previous session I don't remember, I connected to the Pi at 192.168.86.249. Inspected the raw TCP interface data from meshtasticd. Found that `lastHeard` timestamps existed in the raw data but were showing as zero in the GeoJSON output.
