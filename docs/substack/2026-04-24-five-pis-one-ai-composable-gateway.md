...
Five Raspberry Pis in Hawaii run the QA environment that catches these things: **moc**, **fleet-host-1**, **fleet-host-2**, **fleet-host-3**, and **fleet-host**. Each one has a role. moc is the LongFast HAT primary. fleet-host-3 runs a Heltec LoRa32 v3 flashed with RNode firmware on port 903.625 MHz — a real LoRa-over-RNS egress, not a simulation. fleet-host-2 runs ShortTurbo. fleet-host is the broker and sync hub.
